...
public class CalendarMode extends Activity {
	...
	class selectData implements OnClickListener {
		...
		public void onClick(View v) {
			...
			Intent intentOfDetailData = new Intent(CalendarMode.this,DetailData.class);

			String[] plan = mDate[row][column].getText().toString().split("\n");
			String[] sendStartTime = mStartTime[mDate[row][column].getId()]
					.split("\n");
			String[] sendEndTime = mEndTime[mDate[row][column].getId()]
					.split("\n");
	...
	public void displayMonth(int month, int year, int mode) {
		...
		mMonthLabel.setText(String.valueOf(String.valueOf(year + "年" + (month + 1) + "月")));
	...
	public void writePlan(int lastDate, int day, int dayOfWeek, int mode) {
		...
		for (int k = 0; k < lastDate; k++) {
			for (int i = 0; i < mDate.length; i++) {
				for (int j = 0; j < mDate[i].length; j++) {
					int dateNum = mDate[i][j].getText().toString()
							.indexOf("\n");
					if (dateNum != -1) {
						...
						if (checkDate(dummy, date)) {
							if (mDate[i][j].getId() > 0) {
								...
								mDate[i][j].setOnClickListener(new selectData(i, j));
	...
	public void alerm() {
		...
		PendingIntent contentIntent = PendingIntent.getActivity(
				CalendarMode.this, 0, intent, 0);

		Notification notification;
		try {
			Class.forName("android.app.Notification$Builder");
			notification = new Notification.Builder(CalendarMode.this)
			...
			notification.setLatestEventInfo(CalendarMode.this, "contentTitle",
	...
	public void createTable() {
		...
		for (int i = 0; i < mWeek.length; i++) {
			...
			TableRow.LayoutParams lp = new TableRow.LayoutParams(LFP, height/5);
			lp.weight = 1;
			lp.setMargins(1, 1, 1, 1);
			for (int j = 0; j < NUMOFDATE; j++) {
				mDate[i][j] = new TextView(this);
				mDate[i][j].setWidth(getResources().getInteger(R.integer.textWidth));
				mDate[i][j].setHeight(getResources().getInteger(R.integer.textHeight));
