...
public class write_condition_7 extends Activity {
	//目次の項目だけボタンを定義
	...
	private String[] item_pregnancy_writetag = {
	...
	public static int[] item_checkup_8 = {
		R.id.radio_write_condition_day_record,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.write_condition_7);	//画面レイアウトを指定(res/layout/index_read.xml)
					...
					for (int type = parser.getEventType(); type != XmlPullParser.END_DOCUMENT;
						...
						case XmlPullParser.TEXT: // タグの内容
							...
							if(value.trim().length() != 0) {
								...
								Log.d("s",tag);
								Log.d("s",item_pregnancy_writetag[0]);
								//Toast.makeText(this, "なぜかうまく動きませんo((*>∀<))oo((*>∀<))oo((*>∀<))oo((*>∀<))oo((*>∀<))oo((*>∀<))oo((*>∀<))o o((*>∀<))o", Toast.LENGTH_LONG).show();

								if(tag.equals(item_pregnancy_writetag[0])) {
									tvParam[0].setText(value);
									Log.d("a","花咲いiiた");
									}
...
private void button_Write_child_9_12_1_home_onClick() {
	Intent intent_read_2 = new Intent(getApplicationContext(),write_condition_7p.class);
